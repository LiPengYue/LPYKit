...
#import "BaseAnimationProxy.h"
@interface BasePointProgressView()
<
CAAnimationDelegate
>

@property (nonatomic,strong) BaseAnimationProxy <CAAnimationDelegate>*proxy;
...
@implementation BasePointProgressView
...
- (void) setupProperty {
    ...
    self.normalLineStyle = BasePointProgressLineDataMakeNormalDefult();
    self.frontLineStyle = BasePointProgressLineDataMakeHighlightDefult();
}
...
- (void) relayoutViews {
    if ([self.frontLineAnimationDelegate respondsToSelector:@selector(createPointContentViewWithProgressView:)]) {
        [_allPointViewArray enumerateObjectsUsingBlock:^(BasePointProgressContentView * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
...
- (void) relayoutPointViews {
    ...
    [self.allPointViewArray enumerateObjectsUsingBlock:^(BasePointProgressContentView * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
    ...
    [self.allPointViewArray enumerateObjectsUsingBlock:^(BasePointProgressContentView * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        if([self.frontLineAnimationDelegate respondsToSelector:@selector(willDisplayPointView:andIsSected:andIndexPath:)]) {
            CGFloat x = left;
            if (self.selectedType == BasePointProgressSelectedCenter) {
                x = obj.center.x;
            }
...
/// 更新进度
- (void) reloadProgressToViewCenter: (BasePointProgressContentView *) view andOffset: (CGFloat) offsetX {
...
/// 画虚线
- (void) dashLineWithLayer:(CAShapeLayer *)shapeLayer  andStyle: (BasePointProgressLineData)style {
...
- (void) setNormalLineStyle:(BasePointProgressLineData)normalLineStyle {
    _normalLineStyle = normalLineStyle;
    [self relayoutViews];
    [self dashLineWithLayer:self.bgLineLayer andStyle:normalLineStyle];
}

- (void) setfrontLineStyle:(BasePointProgressLineData)frontLineStyle {
    _frontLineStyle = frontLineStyle;
    [self relayoutViews];
    [self dashLineWithLayer:self.frontLineLayer andStyle:frontLineStyle];
}

- (BaseAnimationProxy *)proxy {
    if (!_proxy) {
        _proxy = [[BaseAnimationProxy <CAAnimationDelegate> alloc]initWithTarget:self];
    }
    return _proxy;
}
...
- (BasePointProgressContentView *)currentProgressView {
    if (!_currentProgressView) {
        _currentProgressView = [BasePointProgressContentView new];
...
- (NSRange) getPointViewSelectedMaxCount: (NSInteger) maxCount {
    ...
    if (maxWidth > 0 && currentProgressW > 0) {
        ...
        [self.allPointViewArray enumerateObjectsUsingBlock:^(BasePointProgressContentView * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
            ...
            x = self.selectedType == BasePointProgressSelectedCenter ? obj.center.x : x;
...
@implementation BasePointProgressContentView
- (instancetype)initWithFrame:(CGRect)frame
{
    self = [super initWithFrame:frame];
    if (self) {
