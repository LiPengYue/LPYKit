...
@property (nonatomic,strong) ViewShadowConfigration *shadowConfig;
@end
...
- (void) baseSetupProperty {
    self.bgColor = BaseColorHandler.cBackground;
...
- (BaseFilletShadowViewConfig *) config {
    if (!_config) {
        ...
        _config
        .setUpRadius(0);
        BaseFilletShadowViewConfigPropertyChangedBlock block;
        __weak typeof(self) weakSelf = self;
...
// MARK: lazy loads
- (ViewShadowConfigration *)shadowConfig {
    if (!_shadowConfig) {
        _shadowConfig = ViewShadowConfigration.create(self.shadowLayer);
    }
